...
export class SarifHolder {
  ...
  identifyVulnerability(tool: string, vulnerability_msg: string): object {
    ...
    for (let i = 0; i < tool_vulnerabilities.length; i++) {
      ...
      if (
        vulnerability_msg.includes(vulnerability.Vulnerability as string) ||
        vulnerability.Vulnerability.includes(vulnerability_msg as string)
      ) {
        return {
  ...
  parseLevel(level: string): "none" | "note" | "warning" | "error" {
    ...
    if (
      ...
      level.toLowerCase() == "violations" ||
      level.toLowerCase() == "high"
    ) {
      return "error";
    }
    if (
      level.toLowerCase() == "note" ||
      level.toLowerCase() == "conflicts" ||
      level.toLowerCase() == "informational"
    ) {
      return "note";
    }
